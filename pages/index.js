  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px', minHeight: '100vh', background: '#0a0a0a', color: '#f0f0f0', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        {['all', 'completed', 'in-progress', 'failed'].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            padding: '10px 20px', borderRadius: 24, border: '1px solid #2a2a2a',
            background: filter === f ? '#ff6b00' : '#141414', color: '#fff',
                ...
                <div style={{ marginTop: 18, paddingTop: 18, borderTop: '1px solid #2a2a2a' }}>
                  ...
                  {call.transcript && call.transcript !== 'No transcript available' && (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <div style={{ fontSize: 14, lineHeight: 1.7, color: '#bbb', background: '#0a0a0a', padding: 16, borderRadius: 10, border: '1px solid #222', whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>
                        {call.transcript}
                      </div>
                    </div>
                  )}
                  <div style={{ fontSize: 12, color: '#444', fontFamily: 'monospace', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    <span>ID: {call.id}</span><span>Reason: {call.reason}</span><span>To: {call.to}</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <div style={{ marginTop: 40, padding: 20, background: '#0d0d0d', borderRadius: 16, border: '1px solid #222' }}>
        <div style={{ fontSize: 12, color: '#666', marginBottom: 10, fontWeight: 600 }}>DEBUG: Last Raw Payload from Vapi</div>
        {lastPayload ? (
          <pre style={{ fontSize: 11, color: '#888', overflow: 'auto', maxHeight: 300, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
            {JSON.stringify(lastPayload, null, 2)}
          </pre>
        ) : (
          <div style={{ fontSize: 12, color: '#555' }}>No payload received yet. Make a test call to Phillip.</div>
        )}
      </div>
...
function StatusBadge({ status }) {
  const colors = { completed: '#00c853', 'in-progress': '#ff6b00', failed: '#ff1744', ended: '#888', unknown: '#666' }
  const color = colors[status] || '#888'
  return (
    <span style={{ padding: '5px 12px', borderRadius: 14, background: color + '18', color: color, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, whiteSpace: 'nowrap' }}>
      {status || 'unknown'}
    </span>
  )
}
